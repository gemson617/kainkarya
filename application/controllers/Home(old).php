   ...
   redirect('Auth/logout');
	...
	public function view_certificate()
	{
		if ($this->session->userdata('user_id')) {
			...
			$view_data['user_details'] = $this->mcommon->get_certification($this->session->userdata('user_id'));
	...
	public function pay_success()
	{
	   ...
	   With best regards,<br><br><br>Team, KAINKARYA CHARITABLE TRUST.';
		...
		$this->email->send(); */
			...
			$sms_temp = 'Dear ' . $_POST['firstname'] . ', Thank you for your contribution to service for noble causes. You can download the receipt using your credentials into our website www.kainkarya.com - Kainkarya Charitable Trust.';
	...
	public function view_aid()
	{
		...
		if ($this->session->userdata('auth_level') > 7) {
	...
	public function otpcon()
	{
		...
		$otp = '098765';
		$message = 'Dear Sir/Madam, OTP to login to your KAINKARYA CHARITABLE TRUST portal is' . $otp . '.Welcome to the service for noble causes - Kainkarya Charitable Trust.';
	...
	public function AboutUs()
	{
		...
		$this->load->view('Webpage/about-us.php');
	}
	...
	public function DownloadPamphlet()
	{
		...
		$this->load->view('Webpage/download-pamphlet.php');
	}

	public function DownloadForms()
	{

		$this->load->view('Webpage/download-forms.php');
	}
	...
	public function forgetpassword()
	{
		...
		if (isset($_POST['submit'])) {
			...
			if (empty($check_mail)) {
				...
				$message_mail = 'Dear Sir/Madam, OTP to login to your KAINKARYA CHARITABLE TRUST portal is ' . $otp . '.Welcome to the service for noble causes - Kainkarya Charitable Trust.';
				$this->email->message($message_mail);
				$this->email->send();
	...
	public function UserRegister()
	{
		if (isset($_POST['submit'])) {
			...
			$check_mobile = $this->mcommon->records_all('users', array('email' => $mobile_number,));
			if ($check_mail) {
		...
		if (isset($_POST['submit'])) {
			...
			if ($result) {
				...
				if ($update) {
					...
					$message_mail = 'Dear,' . $firstName . '<br> Thank you for your contribution to service for noble causes. You can download the receipt using your credentials into our website www.kainkarya.com - Kainkarya Charitable Trust.<br> ';
					$this->email->message($message_mail);
					$this->email->send();
	...
	public function polling_submit()
	{
		if (isset($_POST['submit'])) {
			...
			if ($user_auth_level) {
				...
				if ($check_polling) {
					// echo "<pre>";
					// print_r($check_polling);
					// die();
					$this->start_polling($mobile,$user_auth_level,$check_polling);
					
				} else {
					$this->session->set_flashdata('alert_danger', 'Polling Not Available for this role.');
					redirect('home/new_polling/' . $polling_id);
				}
			} else {
	...
	function start_polling($mobile,$user_auth_level,$polling_id){
		$view_data['polling_id'] = $polling_id;
		$view_data['mobile']=$mobile;
		$view_data['user_auth_level']=$user_auth_level;
		$view_data['polling']=$this->mcommon->get_polling_by_id($user_auth_level);
		...
		if($check_polling){
			$view_data['error']="Poll Already Submitted";
	...
	function poll_failure(){
		$view_data['error']="Poll Submission Error";
		$this->load->view('Webpage/poll_failure.php',$view_data);
	}
